 ...
 * 
 */
public class PhotomEqSolverDC5sf {
	...
	private String psmVersion = "v_DC5";
	...
	public void solve() throws Exception {
		...
		while (rs0.next()) {
			...
			r = 0.30*r/3600.;
			//deltamag = deltamag + 0.10*r - 0.05*r*r + 0.025*r*r*r;
			//deltamag = deltamag - 0.10*r*r*r;
		...
		//  (a_1, ..., a_nccd), the three star flat coefficients, 
		//and the first-order extinction (k),
		//  the number of free parameters is just nccd plus four.
		int nparam = 2*nccd + 4;
		int nFreeParam = nccd + 4;
		...
		for (int iteration = 0; iteration < niterations; iteration++) {
			...
			for (iccd = 0; iccd < nccd; iccd++) {
				...
				int iparam_R1 = 1 + 2*nccd;
				int iparam_R2 = 2 + 2*nccd;
				int iparam_R3 = 3 + 2*nccd;
				...
				if (size > 0) {
					for (int j = 0; j < size; j++) {
						...
						for (int iterm = 0; iterm < 3; iterm++) {
							...
							int iparam_R = 1 + 2*nccd + iterm;
							double ipow = iterm + 1.0;
							double r2 = r*r;
							array2d[iparam_R][iparam_R] = array2d[iparam_R][iparam_R] + Math.pow(r2,ipow) * weight;
							//////array2d[iparam_R][iparam_R] = 1.0;
							
							array2d[0][iparam_R] = array2d[0][iparam_R] + Math.pow(r,ipow) * airmass * weight;
							array2d[iparam_R][0] = array2d[iparam_R][0] + Math.pow(r,ipow) * airmass * weight;
							//////array2d[iparam_R][0] = 0.0;
							
							array2d[iparam_a][iparam_R] = array2d[iparam_a][iparam_R] + Math.pow(r,ipow) * weight;
							array2d[iparam_R][iparam_a] = array2d[iparam_R][iparam_a] + Math.pow(r,ipow) * weight;
							//////array2d[iparam_R][iparam_a] = 0.0;
							
							array1d[iparam_R] = array1d[iparam_R] + Math.pow(r,ipow) * deltamag * weight;
							//////if (iterm == 0) {
							//////array1d[iparam_R] = 0.01;
							//////} else {
							//////array1d[iparam_R] = 0.0;
							//////}
						}

						for (int iterm_i = 0; iterm_i < 2; iterm_i++) {
							int iparam_R_i = 1 + 2*nccd + iterm_i;
							double ipow_i = iterm_i + 1.0;
							for (int iterm_j = iterm_i+1; iterm_j < 3; iterm_j++) {
								int iparam_R_j = 1 + 2*nccd + iterm_j;
								double ipow_j = iterm_j + 1.0;
								array2d[iparam_R_i][iparam_R_j] = array2d[iparam_R_i][iparam_R_j] + Math.pow(r,ipow_i)*Math.pow(r,ipow_j) * weight;
								array2d[iparam_R_j][iparam_R_i] = array2d[iparam_R_j][iparam_R_i] + Math.pow(r,ipow_i)*Math.pow(r,ipow_j) * weight;
								//////array2d[iparam_R_i][iparam_R_j] = 0.0;
								//////array2d[iparam_R_j][iparam_R_i] = 0.0;
						...
						if (bsolve) {
							...
							for (int iterm = 0; iterm < 3; iterm++) {								
								int iparam_R = 1 + 2*nccd + iterm;
								int ipow = iterm + 1;
								array2d[iparam_b][iparam_R] = array2d[iparam_b][iparam_R] + Math.pow(r,ipow) * deltaStdColor * weight;
								array2d[iparam_R][iparam_b] = array2d[iparam_R][iparam_b] + Math.pow(r,ipow) * deltaStdColor * weight;
								//////array2d[iparam_R][iparam_b] = 0.0;
							}
			...
			int iparam_R2 = 2 + 2*nccd;
			int iparam_R3 = 3 + 2*nccd;
			R1 = XX.get(iparam_R1, 0);
			R1err = AAinv.get(iparam_R1, iparam_R1);
			if (R1err > 0.) {
				R1err = Math.sqrt(R1err);
			} else {
				R1err = -1;
			}
			R2 = XX.get(iparam_R2, 0);
			R2err = AAinv.get(iparam_R2, iparam_R2);
			if (R2err > 0.) {
				R2err = Math.sqrt(R2err);
			} else {
				R2err = -1;
			}
			R3 = XX.get(iparam_R3, 0);
			R3err = AAinv.get(iparam_R3, iparam_R3);
			if (R3err > 0.) {
				R3err = Math.sqrt(R3err);
			} else {
				R3err = -1;
			}
			...
			if (verbose > 1) {
				System.out.println("R1= " + R1 + " +/- " + R1err);
				System.out.println("R2= " + R2 + " +/- " + R2err);
				System.out.println("R3= " + R3 + " +/- " + R3err);
			...
			for (iccd = 0; iccd < nccd; iccd++) {
				...
				if (size > 0) {
					...
					for (int j = 0; j < size; j++) {
						...
						double r = Math.sqrt(x*x + y*y);
						r = 0.30*r/3600.;
						double res = deltamag - (a[iccd] + k * airmass + b[iccd] * deltaStdColor + R1*r + R2*r*r + R3*r*r*r);
