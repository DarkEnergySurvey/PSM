/**
 * PhotomEqSolverRunYear1.java   The DES Collaboration, 17 October 2012
 *                               Updated: 17 October 2012
 */

package gov.fnal.eag.dtucker.desPhotoStds;

import gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverYear1;
 ...
 *
 */
public class PhotomEqSolverRunYear1 {
	
	private static void printUsage(boolean error) {

		PhotomEqSolverYear1 ph = new PhotomEqSolverYear1();
        
		// Grab default values from PhotomEqSolverYear1...
        ...
        String message = 
				...
				"                                 values for parameters in the paramFile will override values hardwired into the PSMYear1 code \n" +
				...
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 -P BCS -n 20061223 -f g --ccdid 0 --magLo 15.0 --magHi 18.0 --niter 3 --nsigma 2.5 --imageType remap --imageNameFilter % --run 20080324000000_20061223 --psmVersion v_Year1 -v 2 --bsolve --ksolve \n\n" +
				"   Example 2: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 --user myUserName -p myPassword -P BCS -n 20061223 -f g -v 2 --ksolve \n\n" + 
				"   Example 2a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 -P BCS -n 20061223 -f g -v 2 --ksolve \n\n" + 
				"   Example 3: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 --user myUserName -p myPassword -P BCS --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 3a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 -P BCS --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 4: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 --user myUserName -p myPassword -P BCS -n 20061223 -f g -v 2 --ksolve  --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 4a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunYear1 -P BCS -n 20061223 -f g -v 2 --ksolve  --paramFile /home/myname/psmParam.par \n\n" + 
    ...
    public static void main (String[] args) throws Exception {
        ...
        if (localVerbose > 0) {
        	System.out.println("PhotomEqSolverRunYear1 \n");
        }
        ...
        if (localVerbose > 0) {
            ...
            URL classLocation = PhotomEqSolverRunYear1.class.getResource("PhotomEqSolverRunYear1.class");
        ...
        // Instantiate an instance of the PhotomEqSolverYear1 class...
        PhotomEqSolverYear1 ph = new PhotomEqSolverYear1();

        // Grab default values for parameters from PhotomEqSolverYear1...
		String urlDefault             = ph.getUrl();
        String dbNameDefault          = ph.getDbName();
        String userDefault            = ph.getUser();
        String passwdDefault          = ph.getPasswd();
        ...
        // These values take precedence over those hardwired in the PhotomEqSolverYear1 code, 
		...
		// Instantiate a CmdLineParser to read in the arguments pass to PhotomEqSolverRunYear1...
    	...
    	// Set the instance variables for the PhotomEqSolverYear1 object ph using the values 
    	// determined above...
    	if (localVerbose > 0) {
