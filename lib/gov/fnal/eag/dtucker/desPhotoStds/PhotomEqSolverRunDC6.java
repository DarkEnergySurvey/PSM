/**
 * PhotomEqSolverRunDC6.java   The DES Collaboration, 11 August 2011
 *                             Updated: 11 August 2011
 */

package gov.fnal.eag.dtucker.desPhotoStds;

import gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverDC6;
 ...
 *
 */
public class PhotomEqSolverRunDC6 {
	
	private static void printUsage(boolean error) {

		PhotomEqSolverDC6 ph = new PhotomEqSolverDC6();
        
		// Grab default values from PhotomEqSolverDC6...
        ...
        String magTypeDefault         = ph.getMagType();
        ...
        String message = 
       			"\nUsage:  java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 [OPTIONS] \n\n" +
				"   OPTIONS: \n" + 
				"   --paramFile VALUE             name of (optional) parameter file \n" + 
				"                                 values for parameters in the paramFile will override values hardwired into the PSMDC6 code \n" +
				...
				"   -h, --help                    this message \n\n" + 
				"   Example 1: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 --url jdbc:oracle:thin:@charon.ncsa.uiuc.edu:1521: --dbName des  -u myUserName -p myPassword -P BCS -n 20061223 -f g --ccdid 0 --magLo 15.0 --magHi 18.0 --niter 3 --nsigma 2.5 --imageType remap --imageNameFilter % --run 20080324000000_20061223 --psmVersion v_DC6 -v 2 --bsolve --ksolve \n\n" +
				"   Example 1a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 -P BCS -n 20061223 -f g --ccdid 0 --magLo 15.0 --magHi 18.0 --niter 3 --nsigma 2.5 --imageType remap --imageNameFilter % --run 20080324000000_20061223 --psmVersion v_DC6 -v 2 --bsolve --ksolve \n\n" +
				"   Example 2: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 --user myUserName -p myPassword -P BCS -n 20061223 -f g -v 2 --ksolve \n\n" + 
				"   Example 2a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 -P BCS -n 20061223 -f g -v 2 --ksolve \n\n" + 
				"   Example 3: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 --user myUserName -p myPassword -P BCS --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 3a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 -P BCS --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 4: \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 --user myUserName -p myPassword -P BCS -n 20061223 -f g -v 2 --ksolve  --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 4a (with .desdm file): \n" +
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 -P BCS -n 20061223 -f g -v 2 --ksolve  --paramFile /home/myname/psmParam.par \n\n" + 
				"   Example 5 (no .desdm file required): \n" + 
				"      java gov.fnal.eag.dtucker.desPhotoStds.PhotomEqSolverRunDC6 --help \n\n";
    ...
    public static void main (String[] args) throws Exception {
        ...
        if (localVerbose > 0) {
        	System.out.println("PhotomEqSolverRunDC6 \n");
        }
        ...
        if (localVerbose > 0) {
            ...
            URL classLocation = PhotomEqSolverRunDC6.class.getResource("PhotomEqSolverRunDC6.class");
        ...
        // Instantiate an instance of the PhotomEqSolverDC6 class...
        PhotomEqSolverDC6 ph = new PhotomEqSolverDC6();

        // Grab default values for parameters from PhotomEqSolverDC6...
		String urlDefault             = ph.getUrl();
        String dbNameDefault          = ph.getDbName();
        String userDefault            = ph.getUser();
        String passwdDefault          = ph.getPasswd();
        ...
        // These values take precedence over those hardwired in the PhotomEqSolverDC6 code, 
		...
		// Instantiate a CmdLineParser to read in the arguments pass to PhotomEqSolverRunDC6...
        ...
        CmdLineParser.Option magTypeOption         = parser.addStringOption("magType");
    			...
    			if (line.length() >= 1 && line.charAt(0) != '#' ) {
    				...
    				if (localVerbose > 1) {
     				...
     				if (nTokens >= 2) {
     					...
     					} else if (field1.equals("magType")) {
     						magTypeDefault = field2;
     					...
     					//	bdefaultErrDefault = Double.parseDouble(field2);
     						...
     						skipQAPlotsDefault = Boolean.parseBoolean(field2);
    	...
    	String magType = (String)parser.getOptionValue(magTypeOption, magTypeDefault);
    	...
    	// Set the instance variables for the PhotomEqSolverDC6 object ph using the values 
    	// determined above...
    	if (localVerbose > 0) {
    	...
    	ph.setMagType(magType);   
    	if (localVerbose > 0) {System.out.println("magType="+ph.getMagType());}
    	...
    	ph.setSkipQAPlots(skipQAPlots);
    	if (localVerbose > 0) {System.out.println("skipQAPlots="+ph.getSkipQAPlots());}
